...
def login(username: str, password: str) -> (str, requests.session):
    headers = {
        "user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) "
                      "Chrome/87.0.4103.116 Safari/537.36",
...
def notify_user(token: str, msg: str):
    rs = requests.post(url="http://www.pushplus.plus/send", json=dict(token=token, title="Euserv续签", content=msg, channel="wechat")).json()
    ...
    for i in range(len(user_list)):
        ...
        for k, v in SERVERS.items():
            if v:
                ...
                print("ServerID: %s 不需要续订！" % k)
                TOKEN and notify_user(token=TOKEN, msg="ServerID: %s 不需要续订！" % k)
        time.sleep(15)
        check(sessid, s)
        time.sleep(5)
    print('*' * 30)
